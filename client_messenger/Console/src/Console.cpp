...
void Console::help()
{
	...
	system("cls");	
	print("[--account | -a]", "Операции с аккаунтом.\n", FILL);
	print("[--chats   | -c]", "Операции с чатом.\n", FILL);
...
void Console::accountOperation()
{
...
#if(CURRENT_LANGUAGE  == LANGUAGE_RU)
	system("cls");
	print("[--ch name   | -chn]", "Изменить имя\n", FILL);
	...
	print("[--save corr | -scr]", "Изменить настройку сохранения переписок\n", FILL);
	print("[--exit]", "Выход\n", FILL);
	...
	print("[--save corr | -scr]", "Change the settings for saving correspondence\n", FILL);
...
void Console::chatsHelp(map<string, shared_ptr<Chat>> chats)
{
	...
	print("[--select | -s]", "Выбрать чат из списка ниже\n", FILL);
	print("[--back]", "Вернуться назад\n", FILL);
#elif(CURRENT_LANGUAGE == LANGUAGE_EN)
